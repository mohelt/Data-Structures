...
public class HeapPriorityQueue<K,V> extends AbstractPriorityQueue<K,V> {
  ...
  protected void upheap(int j) {
    while(j > 0) {
      int pParent = parent(j);
      if(compare(heap.get(j), heap.get(pParent)) >= 0) {
        break;
      }
      swap(j, pParent); 
      j = pParent; 
  ...
  protected void downheap(int j) {
    while(left(j) < heap.size()) {
      int leftIndexNode = left(j);
      int rightIndexNode = right(j);

      int childIndex = leftIndexNode;
      if(rightIndexNode < heap.size() && compare(heap.get(leftIndexNode), heap.get(rightIndexNode)) > 0) {
        childIndex = rightIndexNode;
      }
  ...
  protected void heapify() {
    int startIndexNode = parent(size() - 1); 
    for (int j=startIndexNode; j >= 0; j--)
      downheap(j);
  }
  ...
  public Entry<K,V> insert(K key, V value) throws IllegalArgumentException {
    Entry<K, V> newestEntry = new PQEntry<K, V>(key, value);

    heap.add(newestEntry);
    upheap(heap.size() - 1);
